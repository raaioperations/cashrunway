...
export async function readState() { const db = await openDatabase(); return new Promise((resolve, reject) => { const tx = db.transaction(STORES, 'readonly'); const state = {}; STORES.forEach(name => { state[name] = []; const req = tx.objectStore(name).getAll(); req.onsuccess = () => { state[name] = req.result; }; }); tx.oncomplete = () => { db.close(); resolve(state); }; tx.onerror = () => { db.close(); reject(tx.error); }; }); }
export async function replaceState(state) { const db = await openDatabase(); return new Promise((resolve, reject) => { const tx = db.transaction(STORES, 'readwrite'); STORES.forEach(name => { const store = tx.objectStore(name); store.clear(); for (const row of state[name] ?? []) store.put(row); }); tx.oncomplete = () => { db.close(); resolve(); }; tx.onerror = () => { db.close(); reject(tx.error); }; }); }
